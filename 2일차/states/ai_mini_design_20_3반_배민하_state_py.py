...
class SourceInfo(TypedDict):
    ...
    ai_confidence_score: Optional[float] # AI가 판단한 해당 정보의 신뢰도 (0.0 ~ 1.0) - 활성화
...
class TechEvaluationOutput(TypedDict):
    ...
    evaluation_confidence_score: Optional[float] # 해당 기술 평가 전반에 대한 AI의 신뢰도
...
class MarketEvaluationOutput(TypedDict):
    ...
    evaluation_confidence_score: Optional[float] # 해당 시장 평가 전반에 대한 AI의 신뢰도
    sources: List[SourceInfo]
...
class TeamEvaluationOutput(TypedDict):
    ...
    evaluation_confidence_score: Optional[float] # 해당 팀 평가 전반에 대한 AI의 신뢰도
    sources: List[SourceInfo]
...
class BizModelEvaluationOutput(TypedDict):
    ...
    evaluation_confidence_score: Optional[float] # 해당 사업 모델 평가 전반에 대한 AI의 신뢰도
    sources: List[SourceInfo]
...
class SingleStartupDetailedAnalysis(TypedDict):
    ...
    weighted_overall_score: Optional[float] # 명시적 계산 로직으로 채워질 예정
...
class AdvancedStartupEvaluationState(TypedDict, total=False):
    ...
    # 예: {'tech': 0.15, 'market': 0.25, 'team': 0.30, 'biz_model_track_record': 0.10, 'biz_model_deal_terms':0.10, 'biz_model_revenue_model': 0.10} (가중치 합 1.0 되도록 조정 필요)
    ...
    # VVVV --- 다음 두 줄을 여기에 추가합니다 --- VVVV
    current_startup_index: Optional[int]
    current_startup_name: Optional[str]
    _temp_next_index: Optional[int]  # <--- 이 필드를 추가합니다.
    # ^^^^ --- 여기까지 추가 --- ^^^^